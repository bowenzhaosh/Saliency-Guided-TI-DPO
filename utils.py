...
def slice_and_move_batch_for_device(batch: Dict, rank: int, world_size: int, device: str) -> Dict:
    ...
    if len(list(batch.values())[0]) == 0:
        raise ValueError("Empty batch passed to slice_and_move_batch_for_device")
    
    # For single GPU, return the whole batch directly
    if world_size == 1:
        on_device = {k: (v.to(device) if isinstance(v, torch.Tensor) else v) for k, v in batch.items()}
        return on_device
    
    # Slicing logic for multi-GPU
    total_size = len(list(batch.values())[0])
    chunk_size = total_size // world_size
    start = chunk_size * rank
    end = chunk_size * (rank + 1) if rank < world_size - 1 else total_size
    
    # Ensure no empty slices
    if start >= total_size:
        raise ValueError(f"Invalid slice: start={start}, total_size={total_size}, rank={rank}, world_size={world_size}")
